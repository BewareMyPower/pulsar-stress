...
@Command(
    name = "get-message-ttl-latency",
    description = "Continuously read topic messageTTLInSeconds and report latency")
public class GetTopicMessageTtlLatencyCommand implements Callable<Integer> {
  ...
  public Integer call() throws Exception {
    ...
    for (int i = 0; i < numOperations; i++) {
      ...
      admin.topicPolicies().getMessageTTL(topicName);
      stats.record(System.nanoTime() - startNanos);
    }

    System.out.printf(
        "get-message-ttl-latency topics=%d rate=%d ops=%d %s%n",
